defmodule Presto.Examples.ComplianceRules do
  ...
  def process_compliance_check(time_entries, rule_spec \\ %{}) do
    ...
    weekly_aggregations = aggregate_entries_by_week(time_entries)
  ...
  def check_weekly_compliance(weekly_aggregation, max_weekly_hours) do
    {:weekly_hours, {employee_id, week_start}, %{total_hours: total_hours} = data} = weekly_aggregation
  ...
  def validate_week_boundaries(time_entries) do
    ...
    |> Enum.all?(fn {_, _, day_of_week} -> day_of_week == 1 end) # Monday is 1
  end
  ...
  defp create_weekly_aggregations(grouped_entries) do
    ...
    |> Enum.map(fn {{emp_id, week_start}, entries} ->
      ...
      week_end = Date.add(week_start, 6) # Sunday
  ...
  defp generate_compliance_summary(weekly_aggregations, compliance_results) do
    ...
    employees_with_violations = 
      compliance_results
      |> Enum.filter(fn {:compliance_result, _, %{status: status}} -> status == :non_compliant end)
      |> Enum.map(fn {:compliance_result, {emp_id, _}, _} -> emp_id end)
      |> Enum.uniq()
      |> length()
    
    compliance_rate = if total_weeks > 0, do: ((total_weeks - violations) / total_weeks * 100) |> Float.round(1), else: 100.0
  ...
  def employee_compliance_stats(compliance_results, employee_id) do
    ...
    violations = Enum.count(employee_results, fn {:compliance_result, _, %{status: status}} -> status == :non_compliant end)
    compliance_rate = if total_weeks > 0, do: ((total_weeks - violations) / total_weeks * 100) |> Float.round(1), else: 100.0
