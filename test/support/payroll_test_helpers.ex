defmodule Presto.PayrollTestHelpers do
  ...
  def create_overtime_work_week(employee_id, monday_date, extra_hours \\ 10) do
    ...
    # Start at 5 PM
    {start_dt, _} = Factories.work_day_hours(friday_date, 17, 0)
  ...
  def time_calculation_accurate?(entry, tolerance_minutes \\ 1) do
    {:time_entry, _, %{start_datetime: start_dt, finish_datetime: finish_dt, minutes: minutes}} =
      entry
  ...
  def create_part_time_week(employee_id, monday_date) do
    ...
    work_days = [
      # Monday
      Date.add(monday_date, 0),
      # Wednesday
      Date.add(monday_date, 2),
      # Friday
      Date.add(monday_date, 4)
    ]
  ...
  defp calculate_entry_units(
         {:time_entry, _, %{start_datetime: start_dt, finish_datetime: finish_dt}}
       ) do
