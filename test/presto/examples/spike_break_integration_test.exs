defmodule Presto.Examples.SpikeBreakIntegrationTest do
  ...
  describe "Real-World Multi-Jurisdiction Integration Scenarios" do
    test "tech company with employees across multiple Bay Area cities" do
      ...
      company_scenario = %{
        ...
        sf_developers: [
          create_crunch_session("dev_sf_001", base_date, "san_francisco", 14, %{role: "senior_dev", team: "backend"}),
          create_crunch_session("dev_sf_002", base_date, "san_francisco", 16, %{role: "frontend_dev", team: "ui"}),
          create_crunch_session("dev_sf_003", base_date, "san_francisco", 12, %{role: "devops", team: "infrastructure"})
        ],

        # Palo Alto office - AI/ML team
        palo_alto_ml: [
          create_crunch_session("ml_pa_001", base_date, "palo_alto", 15, %{role: "ml_engineer", team: "ai"}),
          create_crunch_session("ml_pa_002", base_date, "palo_alto", 13, %{role: "data_scientist", team: "ai"})
        ],

        # Mountain View office - mobile team
        mv_mobile: [
          create_crunch_session("mob_mv_001", base_date, "mountain_view", 11, %{role: "ios_dev", team: "mobile"}),
          create_crunch_session("mob_mv_002", base_date, "mountain_view", 13, %{role: "android_dev", team: "mobile"})
        ]
      }

      # Define jurisdiction for each location
      sf_jurisdiction = %{state: "california", region: "bay_area", city: "san_francisco", industry: "technology"}
      palo_alto_jurisdiction = %{state: "california", region: "bay_area", city: "palo_alto", industry: "technology"}
      mv_jurisdiction = %{state: "california", region: "bay_area", city: "mountain_view", industry: "technology"}

      # Process each location group
      sf_result = CaliforniaSpikeBreakRules.process_spike_break_compliance(
        company_scenario.sf_developers, %{}, sf_jurisdiction
      )

      palo_alto_result = CaliforniaSpikeBreakRules.process_spike_break_compliance(
        company_scenario.palo_alto_ml, %{}, palo_alto_jurisdiction
      )

      mv_result = CaliforniaSpikeBreakRules.process_spike_break_compliance(
        company_scenario.mv_mobile, %{}, mv_jurisdiction
      )
      ...
      Enum.each(all_results, fn result ->
        tech_requirements = SpikeBreakTestHelpers.extract_spike_break_requirements(result.spike_requirements)
        |> Enum.filter(fn {:spike_break_requirement, _, data} ->
          data.type == :bay_area_tech_crunch
        end)

        assert length(tech_requirements) > 0, "Each Bay Area location should have tech crunch requirements"
      end)
    end

    test "entertainment industry during award season across LA County" do
      award_season_date = ~D[2024-02-15] # Peak award season

      # Entertainment companies across LA County during award season
      entertainment_scenario = %{
        # Hollywood studios
        hollywood_productions: [
          create_entertainment_session("prod_hw_001", award_season_date, "hollywood", 18, %{role: "director", project: "feature_film"}),
          create_entertainment_session("prod_hw_002", award_season_date, "hollywood", 16, %{role: "cinematographer", project: "feature_film"}),
          create_entertainment_session("prod_hw_003", award_season_date, "hollywood", 20, %{role: "editor", project: "post_production"})
        ],

        # Burbank studios - TV production
        burbank_tv: [
          create_entertainment_session("tv_bb_001", award_season_date, "burbank", 14, %{role: "showrunner", project: "tv_series"}),
          create_entertainment_session("tv_bb_002", award_season_date, "burbank", 16, %{role: "writer", project: "tv_series"})
        ],

        # Santa Monica - post-production facilities
        santa_monica_post: [
          create_entertainment_session("post_sm_001", award_season_date, "santa_monica", 15, %{role: "sound_engineer", project: "post_production"}),
          create_entertainment_session("post_sm_002", award_season_date, "santa_monica", 17, %{role: "vfx_artist", project: "post_production"})
        ]
      }

      # LA County jurisdiction applies to all locations
      la_jurisdiction = %{state: "california", region: "los_angeles_county", industry: "entertainment"}

      # Process all entertainment work
      all_entertainment_sessions = entertainment_scenario.hollywood_productions ++
                                  entertainment_scenario.burbank_tv ++
                                  entertainment_scenario.santa_monica_post

      result = CaliforniaSpikeBreakRules.process_spike_break_compliance(
        all_entertainment_sessions, %{}, la_jurisdiction
      )
      ...
      entertainment_requirements = SpikeBreakTestHelpers.extract_spike_break_requirements(result.spike_requirements)
      |> Enum.filter(fn {:spike_break_requirement, _, data} ->
    ...
    test "cross-industry mobile workforce scenario" do
      base_date = ~D[2024-08-15] # During harvest season
      ...
      mobile_workforce_scenario = %{
        ...
        agricultural_consultant: SpikeBreakTestHelpers.create_central_valley_agriculture_session(
          "consultant_ag_001", base_date, 12
        ),

        # Same consultant later works for tech company in Bay Area
        tech_consulting: SpikeBreakTestHelpers.create_bay_area_crunch_session(
          "consultant_ag_001", Date.add(base_date, 1), 10 # Next day
        ),

        # Entertainment industry contractor in LA
        entertainment_contractor: SpikeBreakTestHelpers.create_la_entertainment_session(
          "contractor_ent_001", base_date, 16
        ),

        # Same contractor does tech work in Bay Area
        contractor_tech: SpikeBreakTestHelpers.create_bay_area_crunch_session(
          "contractor_ent_001", Date.add(base_date, 2), 14 # Two days later
        ),

        # Regular California employee (no special jurisdiction)
        standard_employee: SpikeBreakTestHelpers.create_extended_day_session(
          "emp_standard_001", base_date, 11
        )
      }
      ...
      entertainment_jurisdiction = %{state: "california", region: "los_angeles_county", industry: "entertainment"}
      standard_jurisdiction = %{state: "california"}

      ag_result = CaliforniaSpikeBreakRules.process_spike_break_compliance(
        [mobile_workforce_scenario.agricultural_consultant], %{}, ag_jurisdiction
      )

      consultant_tech_result = CaliforniaSpikeBreakRules.process_spike_break_compliance(
        [mobile_workforce_scenario.tech_consulting], %{}, tech_jurisdiction
      )

      entertainment_result = CaliforniaSpikeBreakRules.process_spike_break_compliance(
        [mobile_workforce_scenario.entertainment_contractor], %{}, entertainment_jurisdiction
      )

      contractor_tech_result = CaliforniaSpikeBreakRules.process_spike_break_compliance(
        [mobile_workforce_scenario.contractor_tech], %{}, tech_jurisdiction
      )

      standard_result = CaliforniaSpikeBreakRules.process_spike_break_compliance(
        [mobile_workforce_scenario.standard_employee], %{}, standard_jurisdiction
      )

      # Validate cross-industry mobility
      all_results = [ag_result, consultant_tech_result, entertainment_result, contractor_tech_result, standard_result]
      ...
      ag_requirements = SpikeBreakTestHelpers.extract_spike_break_requirements(ag_result.spike_requirements)
      harvest_requirements = Enum.filter(ag_requirements, fn {:spike_break_requirement, _, data} ->
      ...
      |> Enum.each(fn tech_result ->
        tech_requirements = SpikeBreakTestHelpers.extract_spike_break_requirements(tech_result.spike_requirements)
        tech_crunch_requirements = Enum.filter(tech_requirements, fn {:spike_break_requirement, _, data} ->
          data.type == :bay_area_tech_crunch
        end)
    ...
    test "large-scale compliance audit across all California jurisdictions" do
      audit_date = ~D[2024-06-15] # Mid-year audit

      # Simulate statewide compliance audit with representative samples
      statewide_audit = %{
        # Bay Area tech workers
        bay_area_tech: Enum.map(1..10, fn i ->
          create_crunch_session("audit_tech_#{i}", audit_date, "san_francisco", 10 + rem(i, 6), %{audit_id: "tech_audit"})
        end),

        # LA entertainment workers
        la_entertainment: Enum.map(1..8, fn i ->
          create_entertainment_session("audit_ent_#{i}", audit_date, "los_angeles", 12 + rem(i, 8), %{audit_id: "ent_audit"})
        end),

        # Central Valley agricultural workers
        central_valley_agriculture: Enum.map(1..6, fn i ->
          # Not harvest season in June
          SpikeBreakTestHelpers.create_central_valley_agriculture_session("audit_ag_#{i}", audit_date, 8 + rem(i, 4))
        end),

        # San Diego hospitality workers
        san_diego_hospitality: Enum.map(1..5, fn i ->
          create_hospitality_session("audit_hosp_#{i}", audit_date, "san_diego", 9 + rem(i, 3))
        end),

        # Standard California workers (various locations)
        standard_workers: Enum.map(1..15, fn i ->
          SpikeBreakTestHelpers.create_extended_day_session("audit_std_#{i}", audit_date, 8 + rem(i, 5))
        end)
      }

      # Define jurisdictions for audit
      jurisdictions = %{
        bay_area_tech: %{state: "california", region: "bay_area", city: "san_francisco", industry: "technology"},
        la_entertainment: %{state: "california", region: "los_angeles_county", city: "los_angeles", industry: "entertainment"},
        central_valley_agriculture: %{state: "california", region: "central_valley", industry: "agriculture"},
        san_diego_hospitality: %{state: "california", region: "san_diego_county", city: "san_diego", industry: "hospitality"},
        standard_workers: %{state: "california"}
      }

      # Process audit for each jurisdiction
      audit_results = Map.new(jurisdictions, fn {key, jurisdiction} ->
        work_sessions = Map.get(statewide_audit, key)
        result = CaliforniaSpikeBreakRules.process_spike_break_compliance(work_sessions, %{}, jurisdiction)
        {key, result}
      end)
      ...
      assert statewide_summary.total_employees == 44 # 10 + 8 + 6 + 5 + 15
      ...
      compliance_rates = Map.new(audit_results, fn {key, result} ->
    ...
    test "emergency response scenario with relaxed requirements" do
      emergency_date = ~D[2024-07-20] # Summer emergency scenario

      # Simulate emergency response (wildfires, requiring extended work hours)
      emergency_scenario = %{
        # Emergency responders working extended hours
        emergency_responders: [
          create_emergency_session("responder_001", emergency_date, "first_responder", 24), # 24-hour shift
          create_emergency_session("responder_002", emergency_date, "first_responder", 20), # 20-hour shift
          create_emergency_session("responder_003", emergency_date, "first_responder", 18)  # 18-hour shift
        ],
      ...
      emergency_result = CaliforniaSpikeBreakRules.process_spike_break_compliance(
        emergency_scenario.emergency_responders ++ emergency_scenario.utility_workers, %{}, emergency_jurisdiction
      )

      # Process support staff with standard rules
      support_result = CaliforniaSpikeBreakRules.process_spike_break_compliance(
        emergency_scenario.support_staff, %{}, standard_jurisdiction
      )
  ...
  describe "Complex Temporal Integration Scenarios" do
    test "year-long compliance tracking with seasonal variations" do
      ...
      yearly_scenario = [
        ...
        {~D[2024-02-15], :award_season, %{state: "california", region: "los_angeles_county", industry: "entertainment"}, 16},
        ...
        {~D[2024-05-15], :convention, %{state: "california", region: "san_diego_county", city: "san_diego", industry: "hospitality"}, 12},
        {~D[2024-06-15], :standard, %{state: "california"}, 8},

        # Q3 - Harvest season in Central Valley
        {~D[2024-07-15], :harvest_prep, %{state: "california", region: "central_valley", industry: "agriculture"}, 11},
        {~D[2024-08-15], :harvest_peak, %{state: "california", region: "central_valley", industry: "agriculture"}, 12},
        {~D[2024-09-15], :harvest_end, %{state: "california", region: "central_valley", industry: "agriculture"}, 13},

        # Q4 - Tech crunch season
        {~D[2024-10-15], :tech_crunch, %{state: "california", region: "bay_area", city: "san_francisco", industry: "technology"}, 14},
        {~D[2024-11-15], :tech_crunch, %{state: "california", region: "bay_area", city: "san_francisco", industry: "technology"}, 16},
        {~D[2024-12-15], :holiday_standard, %{state: "california"}, 8}
      ]

      # Process each month's work
      yearly_results = Enum.map(yearly_scenario, fn {date, scenario_type, jurisdiction, hours} ->
        work_session = case scenario_type do
          :award_season -> create_entertainment_session(employee_id, date, "los_angeles", hours, %{})
          :convention -> create_hospitality_session(employee_id, date, "san_diego", hours)
          :harvest_prep -> SpikeBreakTestHelpers.create_central_valley_agriculture_session(employee_id, date, hours)
          :harvest_peak -> SpikeBreakTestHelpers.create_central_valley_agriculture_session(employee_id, date, hours)
          :harvest_end -> SpikeBreakTestHelpers.create_central_valley_agriculture_session(employee_id, date, hours)
          :tech_crunch -> create_crunch_session(employee_id, date, "san_francisco", hours, %{})
          _ -> SpikeBreakTestHelpers.create_standard_work_session(employee_id, date, hours)
        end
      ...
      monthly_violations = Enum.map(yearly_results, fn {date, scenario_type, _jurisdiction, result} ->
        {date, scenario_type, result.summary.compliance_violations}
      end)

      # Harvest season (Aug-Sep) should show increased requirements
      harvest_months = Enum.filter(monthly_violations, fn {date, _scenario, _violations} ->
        date.month >= 8 and date.month <= 9
      end)

      tech_crunch_months = Enum.filter(monthly_violations, fn {_date, scenario, _violations} ->
      ...
      # Year-end summary
      total_violations = Enum.reduce(monthly_violations, 0, fn {_date, _scenario, violations}, acc ->
        acc + violations
      end)
    ...
    test "multi-week project with changing jurisdictions" do
      ...
      project_weeks = [
        ...
        {0, "planning", %{state: "california", region: "bay_area", city: "san_francisco"}, 10, false},

        # Week 2: Production in LA
        {7, "production", %{state: "california", region: "los_angeles_county", city: "los_angeles", industry: "entertainment"}, 14, true},

        # Week 3: Post-production in Bay Area
        {14, "post_production", %{state: "california", region: "bay_area", city: "san_francisco", industry: "technology"}, 12, true},

        # Week 4: Delivery in San Diego
        {21, "delivery", %{state: "california", region: "san_diego_county", city: "san_diego"}, 8, false}
      ]

      project_results = Enum.map(project_weeks, fn {day_offset, phase, jurisdiction, hours, is_crunch} ->
        work_date = Date.add(project_start, day_offset)

        work_session = if is_crunch do
          case phase do
            "production" -> create_entertainment_session(employee_id, work_date, "los_angeles", hours, %{phase: phase})
            "post_production" -> create_crunch_session(employee_id, work_date, "san_francisco", hours, %{phase: phase})
            _ -> SpikeBreakTestHelpers.create_extended_day_session(employee_id, work_date, hours)
          end
        else
      ...
      production_result = Enum.find(project_results, fn {phase, _, _, _} -> phase == "production" end)
      post_production_result = Enum.find(project_results, fn {phase, _, _, _} -> phase == "post_production" end)
      ...
      assert production_data.summary.total_spike_break_requirements >= planning_data.summary.total_spike_break_requirements
      assert post_production_data.summary.total_spike_break_requirements >= planning_data.summary.total_spike_break_requirements
    end
  end
  ...
  defp create_crunch_session(employee_id, date, city, hours, metadata \\ %{}) do
    {:work_session, "crunch_#{employee_id}_#{Date.to_string(date)}", %{
      ...
      end_datetime: DateTime.add(DateTime.new!(date, ~T[08:00:00], "Etc/UTC"), hours * 3600, :second),
      total_hours: hours,
  ...
  defp create_entertainment_session(employee_id, date, city, hours, metadata \\ %{}) do
    {:work_session, "entertainment_#{employee_id}_#{Date.to_string(date)}", %{
      ...
      end_datetime: DateTime.add(DateTime.new!(date, ~T[06:00:00], "Etc/UTC"), hours * 3600, :second),
      total_hours: hours,
      meal_breaks_taken: if(hours > 12, do: 2, else: 1),
      breaks_taken: [],
  ...
  defp create_hospitality_session(employee_id, date, city, hours) do
    {:work_session, "hospitality_#{employee_id}_#{Date.to_string(date)}", %{
      ...
      end_datetime: DateTime.add(DateTime.new!(date, ~T[07:00:00], "Etc/UTC"), hours * 3600, :second),
      total_hours: hours,
      meal_breaks_taken: 1,
      breaks_taken: [],
  ...
  defp compile_statewide_audit_summary(audit_results) do
    total_employees = Enum.reduce(audit_results, 0, fn {_key, result}, acc ->
      acc + result.summary.total_employees
    end)

    total_requirements = Enum.reduce(audit_results, 0, fn {_key, result}, acc ->
      acc + result.summary.total_spike_break_requirements
    end)

    total_violations = Enum.reduce(audit_results, 0, fn {_key, result}, acc ->
      acc + result.summary.compliance_violations
    end)

    overall_compliance_rate = if total_requirements > 0 do
  ...
  defp generate_compliance_recommendations(audit_results) do
    ...
    recommendations = Enum.reduce(audit_results, recommendations, fn {key, result}, acc ->
      violation_rate = if result.summary.total_spike_break_requirements > 0 do
      ...
      cond do
        violation_rate > 0.5 and String.contains?(Atom.to_string(key), "tech") ->
          acc ++ ["Consider implementing automated break reminders for technology workers during crunch periods"]

        violation_rate > 0.4 and String.contains?(Atom.to_string(key), "entertainment") ->
          acc ++ ["Review entertainment industry scheduling practices to ensure adequate break coverage"]
