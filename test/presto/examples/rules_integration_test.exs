defmodule Presto.Examples.RulesIntegrationTest do
  ...
  describe "JSON rule specification integration" do
        ...
        Enum.flat_map(0..4, fn day ->
          ...
          [
            Factories.build_processed_time_entry(
              ...
              DateTime.new!(date, ~T[18:00:00], "Etc/UTC"),
              "emp_001"
            )
          ]
        end)
    ...
    test "combined payroll and compliance workflow" do
        ...
        "payroll" => %{
          "rules" => [
            ...
            %{
              ...
              "type" => "payroll",
              # UK standard
              "variables" => %{"overtime_threshold" => 37.5}
            }
          ]
        },
        "compliance" => %{
      ...
      raw_time_entries = [
        ...
        Factories.build_time_entry(
          ...
          DateTime.new!(monday, ~T[17:30:00], "Etc/UTC"),
          "emp_uk_001"
        ),
        ...
        Factories.build_time_entry(
          ...
          DateTime.new!(Date.add(monday, 1), ~T[17:30:00], "Etc/UTC"),
          "emp_uk_001"
        ),
        ...
        Factories.build_time_entry(
          ...
          DateTime.new!(Date.add(monday, 2), ~T[17:30:00], "Etc/UTC"),
          "emp_uk_001"
        ),
        ...
        Factories.build_time_entry(
          ...
          DateTime.new!(Date.add(monday, 3), ~T[17:30:00], "Etc/UTC"),
          "emp_uk_001"
        ),
        ...
        Factories.build_time_entry(
          ...
          DateTime.new!(Date.add(monday, 4), ~T[18:00:00], "Etc/UTC"),
          "emp_uk_001"
        )
      ]

      # Total: 43.0 hours (5.5 hours overtime over 37.5 threshold, but compliant with 48h max)

      # Step 1: Process payroll
      payroll_result =
        PayrollRules.process_time_entries(raw_time_entries, combined_spec["payroll"])

      # Step 2: Process compliance using payroll results
      processed_entries = payroll_result.processed_entries

      compliance_result =
        ComplianceRules.process_compliance_check(processed_entries, combined_spec["compliance"])
      ...
      assert length(overtime_entries) == 1
      [overtime_entry] = overtime_entries
      assert {:overtime_entry, {"emp_uk_001", ^monday}, %{units: 5.5}} = overtime_entry
      assert payroll_summary.total_overtime_hours == 5.5
      ...
      assert length(compliance_results) == 1
      [compliance_entry] = compliance_results
    ...
    test "multi-employee, multi-week scenario with variable thresholds" do
      ...
      complex_spec = %{
        ...
        "employee_overrides" => %{
          "emp_manager_001" => %{
            # Managers have higher threshold
      ...
      standard_emp_entries =
        PayrollTestHelpers.create_overtime_work_week("emp_standard_001", base_monday, 8) ++
      ...
      manager_entries =
        PayrollTestHelpers.create_overtime_work_week("emp_manager_001", base_monday, 15) ++
          PayrollTestHelpers.create_overtime_work_week("emp_manager_001", week2_monday, 12)

      # Part-time employee - always under thresholds
      part_time_entries =
        PayrollTestHelpers.create_part_time_week("emp_parttime_001", base_monday) ++
      ...
      compliance_result =
        ComplianceRules.process_compliance_check(processed_entries, complex_spec)
      ...
      # Manager has 2 weeks over 50-hour threshold (employee overrides not yet implemented)
      assert compliance_summary.compliance_violations == 2
      assert compliance_summary.compliance_rate_percent == 66.7

      # Verify we have data for both weeks
      # 3 employees × 2 weeks
      assert compliance_summary.total_weeks_checked == 6
    end
  end
  ...
  describe "performance with complex specifications" do
    test "processes large rule specifications efficiently" do
      ...
      time_entries =
        for emp_id <- 1..10 do
          PayrollTestHelpers.create_standard_work_week(
      ...
      {payroll_time, payroll_result} =
        :timer.tc(fn ->
          PayrollRules.process_time_entries(time_entries, large_spec)
        end)

      {compliance_time, compliance_result} =
        :timer.tc(fn ->
      ...
      # < 0.5 seconds
      assert payroll_time < 500_000
      # < 0.5 seconds
      assert compliance_time < 500_000
