defmodule Presto.Benchmarks.BenchmarkIntegrationTest do
  ...
  describe "benchmark runner integration" do
    test "can run individual benchmarks", %{engine: engine} do
      # Test alpha network throughput benchmark
      result = BenchmarkRunner.run_benchmark(:alpha_network_throughput, %{
        iterations: 5,
        warmup_iterations: 2,
        data_size: :small
      })
    ...
    test "can run benchmark suite", %{engine: engine} do
      benchmark_names = [:alpha_network_throughput, :rule_engine_execution]
      
      results = BenchmarkRunner.run_benchmark_suite(benchmark_names, %{
        iterations: 3,
        data_size: :small
      })
    ...
    test "can run optimization comparison", %{engine: engine} do
      ...
      result = BenchmarkRunner.run_optimization_comparison(
  ...
  describe "performance monitoring integration" do
    ...
    test "can start and stop performance monitoring" do
      {:ok, monitor} = PerformanceMonitor.start_link(%{
        monitoring_interval: 1000,  # 1 second for testing
  ...
  describe "end-to-end optimization workflow" do
    @tag :slow
    test "complete optimization measurement workflow", %{engine: engine} do
      ...
      optimization_fn = fn ->
        ...
        :timer.sleep(10)  # Simulate optimization work
      end
      
      # Step 3: Run optimization comparison
      comparison_result = BenchmarkRunner.run_optimization_comparison(
        :rule_engine_execution,
        optimization_fn,
        baseline_config
      )
      ...
      analysis = ResultAnalyzer.analyze_optimization_comparison(
  ...
  defp create_complex_rule_set do
    [
      %{
        id: :young_adult,
        conditions: [
        ...
        action: fn facts -> [{:young_adult, facts[:name]}] end,
        priority: 5
      },
      %{
        id: :employed_person,
        ...
        action: fn facts -> [{:employed, facts[:name], facts[:company]}] end,
        priority: 3
      },
      %{
        id: :tech_worker,
